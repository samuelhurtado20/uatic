...
class partidaController extends Controller{
    ...
    public function listar(){
        ...
        Session::acceso('proyecto');
            ...
            $term = $_GET['term'];
            $row = $this->_partidas->listar($term);
            for($i=0; $i < count($row); $i++){ 
        
        $results[] = array('label' => $row[$i]['descripcion'],'unidad' => $row[$i]['unidad_medida'],'id_partidas' => $row[$i]['id_partidas'],'precio' => $row[$i]['precio_unitario']);
